...
inner join images images2_ on product0_.id=images2_.images_id
inner join product_details productdet3_ on product0_.product_details_id=productdet3_.id
where productdet3_.price between 0 and 100000 order by productdet3_.price asc limit ?
...
from product product0_
inner join categories category1_ on product0_.category_id=category1_.id
inner join images images2_ on product0_.id=images2_.product_id
inner join product_details productdet3_ on product0_.product_details_id=productdet3_.id
 where (category1_.name in (?))
 and (product0_.lens_color_id in (? , ?))
 and (product0_.frame_color_id in (? , ?))
 and (productdet3_.price between 50 and 2000)
 order by productdet3_.price desc limit ?